...
class PayrollController extends Controller {
    ...
    public function show() {
        ...
        foreach ($res as $row) {
            ...
            if (isset($row->expense)) {
                ...
                if ($expense) {
                    $operate = BootstrapTableService::button('fa fa-file-o', http_url('payroll/slip/'.$expense->id), ['btn-gradient-info'], ['title' => trans("slip"),'target' => '_blank']);
                    $status = 1;
    ...
    public function slip_list()
    {
        ...
        foreach ($res as $row) {
            $operate = BootstrapTableService::button('fa fa-file-o', http_url('payroll/slip/'.$row->id), ['btn-gradient-info'], ['title' => trans("slip"),'target' => '_blank']);
            $tempRow = $row->toArray();
            $tempRow['no'] = $no++;
            $tempRow['operate'] = $operate;
            $rows[] = $tempRow;
