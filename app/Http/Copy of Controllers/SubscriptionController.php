...
class SubscriptionController extends Controller
{
        ...
        try {
            ...
            if ($type == 0) {
                ...
                $response = [
                    ...
                    'type' => 'prepaid',
                    'url'    => http_url('subscriptions/prepaid/package').'/'.$package_id.'/0/'.$isCurrentPlan
                ];
                return response()->json($response);
            } else { // Postpaid plans
    ...
    public function show()
    {
        ...
        foreach ($res as $row) {
            ...
            $operate .= BootstrapTableService::button('fa fa-file-pdf-o', http_url('subscriptions/bill/receipt', $row->id), ['btn-gradient-info'], ['title' => 'Receipt', 'target' => '_blank']);
    ...
    public function subscription_report_show(Request $request, $status = null)
    {
        ...
        foreach ($res as $row) {
            ...
            if (($row->status == 'Paid' || $row->status == 'Over Due' || $row->status == 'Failed' || $row->status == 'Pending' || $row->status == 'Unpaid' || $row->status == 'Bill Not Generated') && $row->subscription_bill) {
                if ($row->status != 'Paid') {
                    $operate .= BootstrapTableService::menuButton('receive_payment',http_url('subscriptions/bill-payment', $row->subscription_bill->id),['receive_payment'],[]);
                }

                $operate .= BootstrapTableService::menuButton('Receipt',http_url('subscriptions/bill/receipt', $row->subscription_bill->id),[],['target' => '_blank']);
            }
            ...
            if ($row->subscription_bill) {
                if ($row->subscription_bill->transaction) {
                    if (in_array($row->subscription_bill->transaction->payment_gateway, ['Cash','Cheque'])) {
                        ...
                        // $operate .= BootstrapTableService::button('fa fa-dollar', http_url('subscriptions/bill-payment', $row->subscription_bill->id), ['btn-gradient-success receive_payment'], ['title' => trans("edit_payment")]);
                        $operate .= BootstrapTableService::menuButton('edit_payment',http_url('subscriptions/bill-payment', $row->subscription_bill->id),['receive_payment'],[]);

                        // $operate .= BootstrapTableService::deleteButton(http_url('subscriptions/bill-payment/destroy', $row->subscription_bill->payment_transaction_id));
                        $operate .= BootstrapTableService::menuDeleteButton('delete_payment',http_url('subscriptions/bill-payment/destroy', $row->subscription_bill->payment_transaction_id),['receive_payment'],[]);
                    }
                }
            }
        ...
        try {
            ...
            if ($type == 0) {
                ...
                $response = [
                    ...
                    'type' => 0,
                    'url' => http_url('subscriptions/prepaid/package').'/'.$id.'/'.$type .'/'. $isCurrentPlan
                ];
